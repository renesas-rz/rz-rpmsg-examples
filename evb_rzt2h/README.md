# RZ/T2H Cortex®-R52 / Cortex®-A55 RPMsg example
...
## Table of Contents
- [RZ/T2H Cortex®-R52 / Cortex®-A55 RPMsg example](#rzt2h-cortex-r52--cortex-a55-rpmsg-example)
...
|----|----|----|
|1|gcc/iar_rzt2h_ca55_1_rpmsg_linux_baremetal_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|2|gcc/iar_rzt2h_ca55_1_rpmsg_linux_freertos_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|3|gcc/iar_rzt2h_ca55_2_rpmsg_linux_baremetal_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|4|gcc/iar_rzt2h_ca55_2_rpmsg_linux_freertos_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|5|gcc/iar_rzt2h_ca55_3_rpmsg_linux_baremetal_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|6|gcc/iar_rzt2h_ca55_3_rpmsg_linux_freertos_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|7|gcc/iar_rzt2h_cr52_0_rpmsg_linux_baremetal_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|8|gcc/iar_rzt2h_cr52_0_rpmsg_linux_freertos_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|9|gcc/iar_rzt2h_cr52_1_rpmsg_linux_baremetal_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|10|gcc/iar_rzt2h_cr52_1_rpmsg_linux_freertos_demo|Sample project to check the Inter-Processor Communication in multi-OS environment.
|11|rzt2h_ca55_0_baremetal_master_ca55_1_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|12|rzt2h_ca55_0_baremetal_master_ca55_2_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|13|rzt2h_ca55_0_baremetal_master_ca55_3_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|14|rzt2h_ca55_0_baremetal_master_cr52_0_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|15|rzt2h_ca55_0_baremetal_master_cr52_1_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|16|rzt2h_ca55_0_freertos_master_ca55_1_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|17|rzt2h_ca55_0_freertos_master_cr52_0_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|18|rzt2h_ca55_1_baremetal_master_ca55_2_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|19|rzt2h_ca55_1_baremetal_master_cr52_1_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|20|rzt2h_ca55_1_freertos_master_ca55_2_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|21|rzt2h_ca55_1_freertos_master_cr52_1_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|22|rzt2h_cr52_0_baremetal_master_ca55_0_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|23|rzt2h_cr52_0_baremetal_master_ca55_1_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|24|rzt2h_cr52_0_baremetal_master_ca55_2_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|25|rzt2h_cr52_0_baremetal_master_ca55_3_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|26|rzt2h_cr52_0_baremetal_master_cr52_1_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|27|rzt2h_cr52_0_baremetal_master_cr52_1_freertos_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|28|rzt2h_cr52_0_freertos_master_cr52_1_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|29|rzt2h_cr52_1_baremetal_master_cr52_0_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.
|30|rzt2h_cr52_1_freertos_master_cr52_0_baremetal_slave|Sample project to check the Inter-Processor Communication between two FSP cores in a multi-core FSP environment.

### Development Environment
|Hardware|Version|
|----|----|
|[RZ/T2H-EVKIT](https://www.renesas.com/en/design-resources/boards-kits/rz-t2h-evkit)|-|
...
1. `<device>_<platform_type>_<role>_<device>_<platform_type>_<role>` (for FSP ↔ FSP communication)
    - `device`: rzt2h_cr52_0 / rzt2h_cr52_1 / rzt2h_ca55_0 / rzt2h_ca55_1 / rzt2h_ca55_2 / rzt2h_ca55_3
...
### Hardware setup
1. Connect the J-Link OB or I-Jet debugger and a USB-to-serial cable to the RZ/T2H. For details, please refer to [Getting Started with Flexible Software Package](https://www.renesas.com/en/document/apn/rzt2-rzn2-getting-started-flexible-software-package)

2. Setup operation mode switch setting for RZ/T2H Evaluation Board.
- For FSP - FSP combinations.

|Switch|Setting|Description|
|----|----|----|
|SW14-1|ON|xSPI1 boot mode (x1 boot serial flash).
|SW14-2|OFF|xSPI1 boot mode (x1 boot serial flash).
|SW14-3|ON|xSPI1 boot mode (x1 boot serial flash).
...
|----|----|----|
|SW14-1|ON|xSPI1 boot mode (x1 boot serial flash).
|SW14-2|ON|xSPI1 boot mode (x1 boot serial flash).
|SW14-3|ON|xSPI1 boot mode (x1 boot serial flash).
|SW14-4|OFF|CPU0 ATCM wait cycle = 1 wait cycle.
|SW14-5|OFF|CPU1 ATCM wait cycle = 1 wait cycle.
|SW14-6|OFF|Supply voltage of boot peripheral is 3.3 V.
|SW14-7|ON|JTAG mode = Normal mode.
|SW2-3|ON|Signal Connections of MicroSD Card Slot.
|SW5-3|OFF|Signal Connections of MicroSD Card Slot.
|SW5-4|OFF|Signal Connections of MicroSD Card Slot.
...
1. Configure the switches according to Section **Hardware setup** to select either the **FSP - FSP** or **Linux - FSP** combination. If the Linux - FSP combination is selected, wait until the Linux boot process is complete and the message shown below appears before proceeding:
    ```
    Poky (Yocto Project Reference Distro) 5.0.11 rzt2h-dev ttySC0
    rzt2h-dev login:
    ```
...
- For FSP - FSP combinations:
    ...
    - In the case where CR52_0 or CA55_0 is neither a slave core nor a master core, rzt2h_cr52_0_boot/rzt2h_ca55_0_boot must run before other cores begin execution.
...
- For Linux- FSP combinations:
    - For the sample program of the CR52_1/ CA55_1/ CA55_2/ CA55_3 cores, the rzt2h_cr52_0_boot project only needs to complete the project build process and does not need to be downloaded to the board.
...
2. Invoke the command below to specify RPMsg sample program to be loaded:
    ```
    root@rzt2h-dev:~# echo <env>_<device>_rpmsg_linux_<platform_type>_demo.<extension> > /sys/class/remoteproc/<remoteprocX>/firmware
    ```
   - `<env>`: gcc / iar
...
3. Turn on the RZ/T2H Evaluation Board using the POWER_SW. To start invoking the sample from U-Boot, press the Reset button. Then, you should see the following message on the console
    ```
...
2. After rewriting the bootloader files (bl2_bp_xspi0-rzt2h-dev.srec and fip-rzt2h-dev.srec), proceed to program the FSP image here:
    ```
...
4. When Linux receives the echo reply, the message below should be displayed in the console connected to USB to Serial Port of RZ/T2H Evaluation Board.
```
echo test: sent: 165
