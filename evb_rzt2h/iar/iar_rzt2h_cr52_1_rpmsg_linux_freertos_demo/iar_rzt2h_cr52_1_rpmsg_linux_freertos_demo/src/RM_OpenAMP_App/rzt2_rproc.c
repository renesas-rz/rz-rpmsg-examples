 ...
 * DESCRIPTION
 *
 *       This file defines RZ/T2 remoteproc implementation.
 *
 * @par     History
 *          - rev 1.0 (2024.05.31)
...
{
    unsigned int val = 0U;
#ifdef FREERTOSCONFIG_H_               /* FreeRTOS */
    ...
    {
        message_Status = METAL_IRQ_NOT_HANDLED; /* Invalid message arrived */
    }

    ipi.notify_id = val;
    atomic_flag_clear(&ipi.sync);

    message_Status = METAL_IRQ_HANDLED;
}

static int r_icu_inter_cpu_irq_isr_wrapper (int irq, void * context)
{
    (void) irq;
    (void) context;

    r_icu_inter_cpu_irq_isr();
...
static int rzt2_enable_interrupt (struct remoteproc * rproc, struct metal_device * ipi_dev)
{
    ...
    if (!ipi.registered)
    {
        /* Register interrupt handler and enable interrupt for RZ/T2 CA55 or CR52 */
        irq_vect = (uintptr_t) ipi_dev->irq_info;
        ret      = metal_irq_register((int) irq_vect, r_icu_inter_cpu_irq_isr_wrapper, ipi_dev, rproc);
        if (ret)
        {
...
static struct remoteproc * rzt2_proc_init (struct remoteproc * rproc, struct remoteproc_ops * ops, void * arg)
{
...
static int rzt2_proc_notify (struct remoteproc * rproc, uint32_t id)
{
    ...
    *((uint32_t *) (SHM_DEV_BASE + SHM_TX_OFFSET(MBX_TX_CH))) = prproc->notify_id;
    g_icu_inter_cpu_irq1.p_api->generate(g_icu_inter_cpu_irq1.p_ctrl);
#endif
