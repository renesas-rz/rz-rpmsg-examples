...
## Table of Contents
- [RZ/G2LC Cortex-M33 RPMsg example](#rzg2lc-cortex-m33-rpmsg-example)
  - [Table of Contents](#table-of-contents)
  - [Getting Started](#getting-started)
    - [Sample Projects](#sample-projects)
    - [Development Environment](#development-environment)
  - [example rzg2lc\_cm33\_rpmsg\_linux-rtos\_example](#example-rzg2lc_cm33_rpmsg_linux-rtos_example)
...
|----|----|----|
|1|rzg2lc_cm33_rpmsg_linux-rtos_example|Sample project to check the Inter-Processor Communication in multi-OS environment.
...
## Demo rzg2lc_cm33_rpmsg_linux-rtos_example

The **rzg2lc_cm33_rpmsg_linux-rtos_example** outputs the message tranfer progress between CPU cores to the console.
...
4. Connect SEGGER J-Link to RZ/G2LC SMARC EVK. For details, please refer to [Getting Started with Flexible Software Package](https://www.renesas.com/en/document/apn/rzg2l-getting-started-with-flexible-software-package)
...
- rzg2lc_cm33_rpmsg_linux-rtos_example.elf
- rzg2lc_cm33_rpmsg_linux-rtos_example_non_secure_code.bin
- rzg2lc_cm33_rpmsg_linux-rtos_example_non_secure_vector.bin
- rzg2lc_cm33_rpmsg_linux-rtos_example_secure_code.bin
- rzg2lc_cm33_rpmsg_linux-rtos_example_secure_vector.bin

### CM33 sample program invocation
#### CM33 sample program invocation with SEGGER J-Link
1. Start debug project by choosing **rzg2lc_cm33_rpmsg_linux-rtos_example Debug_Flat** or **rzg2lc_cm33_rpmsg_linux-rtos_example Release_Flat**
...
5. Load the binary files you copied at step 1 from microSD card to RAM by executing the commands stated below on the console. Here, **N** stands for the partition number in which you stored the binary files.
    ...
    dcache off
    mmc dev 1
    fatload mmc 1:N 0x0001FF80 rzg2lc_cm33_rpmsg_linux-rtos_example_secure_vector.bin
    fatload mmc 1:N 0x42EFF440 rzg2lc_cm33_rpmsg_linux-rtos_example_secure_code.bin
    fatload mmc 1:N 0x00010000 rzg2lc_cm33_rpmsg_linux-rtos_example_non_secure_vector.bin
    fatload mmc 1:N 0x40010000 rzg2lc_cm33_rpmsg_linux-rtos_example_non_secure_code.bin
...
1. Booting up Linux by following **5. Booting and Running Linux** of [SMARC EVK of RZ/G2LC Linux Startup Guide](https://www.renesas.com/en/document/gde/smarc-evk-rzg2l-rzg2lc-rzg2ul-linux-start-guide-rev107).
2. Invoke the command stated below to specify the sample program to be loaded:
    ```
    root@smarc-rzg2lc:~# echo rzg2lc_cm33_rpmsg_linux-rtos_example.elf > /sys/class/remoteproc/remoteproc0/firmware
    ```
3. Trigger CM33 execution by invoking the command below:
    ```
    root@smarc-rzg2lc:~# echo start > /sys/class/remoteproc/remoteproc0/state
    ```
    ...
    [   93.796548] remoteproc remoteproc0: Booting fw image rzg2lc_cm33_rpmsg_linux-rtos_example.elf, size 1032012
...
3. Run CA55 sample program by executing the following command:
    ```
    root@smarc-rzg2lc:~# rpmsg_sample_client
    ```
...
5. After the message which has 488 bytes sized payload is sent from CA55 to CM33 and CM33 sends back the echo reply, the message for terminating the communication channel is sent from CA55 to CM33. Then, CA55 and CM33 sample programs output the following log messages to the corresponding consoles respectively when receiving the termination message.
  - On CA55 side:
    ...
    * rpmsg communication sample program *
    ******************************************
    1. communicate with RZ/G2LC CM33 ch0
    2. communicate with RZ/G2LC CM33 ch1
    e. exit
    please input
    >
    ```
