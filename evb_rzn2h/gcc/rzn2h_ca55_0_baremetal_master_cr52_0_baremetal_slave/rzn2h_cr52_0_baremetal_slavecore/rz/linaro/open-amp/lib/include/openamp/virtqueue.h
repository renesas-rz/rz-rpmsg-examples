...
typedef void vq_notify (struct virtqueue *);
  ...
  #define VQASSERT(_vq, _exp, _msg)                                                    \
    do {                                                                               \
        if (!(_exp)) {                                                                 \
            metal_log(METAL_LOG_EMERGENCY, "%s: %s - _msg", __func__, (_vq)->vq_name); \
            metal_assert(_exp);                                                        \
        }                                                                              \
    } while (0)
  ...
  #define VQUEUE_BUSY(vq)                                               \
    do {                                                                \
        if (!(vq)->vq_inuse) {                                          \
            ...
            VQASSERT(vq, !(vq)->vq_inuse, "VirtQueue already in use");} \
    } while (0)
 ...
 */
static inline void virtqueue_set_shmem_io (struct virtqueue * vq, struct metal_io_region * io)
{
    vq->shm_io = io;
}
