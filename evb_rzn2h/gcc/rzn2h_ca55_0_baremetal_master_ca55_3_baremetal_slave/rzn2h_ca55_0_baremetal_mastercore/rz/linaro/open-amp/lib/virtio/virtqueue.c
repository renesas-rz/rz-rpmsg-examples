...
static inline void * virtqueue_phys_to_virt (struct virtqueue * vq, metal_phys_addr_t phys)
{
...
static inline metal_phys_addr_t virtqueue_virt_to_phys (struct virtqueue * vq, void * buf)
{
    struct metal_io_region * io = vq->shm_io;
                          ...
                          void                 * cookie)
{
    ...
    if (status == VQUEUE_SUCCESS)
    {
        ...
        idx = vq_ring_add_buffer(vq, vq->vq_ring.desc, head_idx, buf_list, readable, writable);
