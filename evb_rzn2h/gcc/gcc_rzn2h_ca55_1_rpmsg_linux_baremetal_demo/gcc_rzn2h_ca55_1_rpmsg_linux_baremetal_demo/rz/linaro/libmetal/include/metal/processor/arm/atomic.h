...
static inline atomic_flag __sync_lock_test_and_set (volatile atomic_flag * ptr, atomic_flag value) {
    atomic_flag old_value = *ptr;
    *ptr = value;

    return old_value;
}

static inline void __sync_lock_release (volatile int * ptr) {
    *ptr = 0;
}

static inline int __sync_val_compare_and_swap (volatile int * ptr, int oldval, int newval) {
    int prev = *ptr;
    if (prev == oldval)
    {
        *ptr = newval;
...
static inline void __sync_synchronize () {
 ...
 #define atomic_exchange(OBJ, DES)                                  \
    ({                                                              \
        ...
        do {                                                        \
            expval = oldval;                                        \
            oldval = __sync_val_compare_and_swap(obj, expval, des); \
        } while (oldval != expval);                                 \
        oldval;                                                     \
    })
 ...
 #define atomic_compare_exchange_strong(OBJ, EXP, DES)                             \
    ({                                                                             \
        ...
        __typeof__(*obj) oldval = __sync_val_compare_and_swap(obj, expval, (DES)); \
        *exp = oldval;                                                             \
        oldval == expval;                                                          \
    })
