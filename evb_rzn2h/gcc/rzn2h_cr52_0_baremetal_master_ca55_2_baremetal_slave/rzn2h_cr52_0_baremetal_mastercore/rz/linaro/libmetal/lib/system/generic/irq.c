...
int metal_irq_register (int irq, metal_irq_handler hd, struct metal_device * dev, void * drv_id)
{
    ...
    metal_list_for_each(&_irqs.irqs, node)
    {
        ...
        if (irq_p->irq == irq)
        {
            ...
            {
                hdl_p = metal_container_of(h_node, struct metal_irq_hddesc, node);
    ...
    if (hdl_p == NULL)
    {
        metal_log(METAL_LOG_ERROR, "%s: irq %d cannot allocate mem for drv_id %d.\n", __func__, irq, drv_id);
        metal_mutex_release(&_irqs.irq_lock);
...
int metal_irq_unregister (int irq, metal_irq_handler hd, struct metal_device * dev, void * drv_id)
{
    ...
    metal_list_for_each(&_irqs.irqs, node)
    {
        ...
        if (irq_p->irq == irq)
        {
            ...
            {
                metal_log(METAL_LOG_DEBUG, "%s: handlers list empty, unregister interrupt\n", __func__);
