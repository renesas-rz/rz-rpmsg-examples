 ...
 * Prototypes
 */
static void mbed_minimal_formatted_string_integer(char                * buffer,
                                                  size_t                length,
                                                  int                 * result,
                                                  MBED_UNSIGNED_STORAGE value,
                                                  integer_type_t        type,
                                                  int                   width_size,
                                                  bool                  prepend_zeros,
                                                  console_t             console);
static void mbed_minimal_formatted_string_void_pointer(char       * buffer,
                                                       size_t       length,
                                                       int        * result,
                                                       const void * value,
                                                       console_t    console);
static void mbed_minimal_formatted_string_string(char       * buffer,
                                                 size_t       length,
                                                 int        * result,
                                                 const char * string,
                                                 size_t       precision,
 ...
 * @param      type             The type of integer format that shall be printed (signed, unsigned or hexadecimal)
 ...
 * @param[in]  console         The method of char data control.
 */
static void mbed_minimal_formatted_string_double (char    * buffer,
                                                  size_t    length,
                                                  int     * result,
                                                  double    value,
                                                  int       dec_precision,
                                                  ...
                                                  console_t console)
{
    ...
    {
        mbed_minimal_formatted_string_integer(buffer,
                                              length,
                                              result,
                                              integer,
                                              ZERO_NEGATIVE,
 ...
 */
int mbed_minimal_formatted_string (char       * buffer,
                                   size_t       length,
                                   const char * format,
                                   va_list      arguments,
                                   console_t    console)
    ...
    if (format && (length <= INT_MAX))
    {
        ...
        for (size_t index = 0; format[index] != '\0'; index++)
        {
            ...
            if (format[index] == '%')
            {
                 ...
                 * '\0', or a not supported character
                 *************************************************************/
                ...
                else if ((next == 'f') || (next == 'F') || (next == 'g') || (next == 'G'))
                {
                    ...
                    mbed_minimal_formatted_string_double(buffer,
                                                         length,
                                                         &result,
                                                         value,
                                                         precision,
                ...
                {
                    // Unrecognized, or `%%`. Print the `%` that led us in.
                    mbed_minimal_putchar(buffer, length, &result, '%', console);
                    if (next == '%')
                    {
                        // Continue printing loop after `%%`
                    ...
                    // unrecognized thing like "Blah = %a" will just come out as "Blah = %a"
                }
            }
            else
